import { isWeverseLogin, CommunityArray, ArtistArray, NotificationArray, isNotification, PostArray, Post, isPost, NotifContent, NotifKeys, CommentArray, isComment } from "../types";
import { WeverseUrl as urls, validateStatus, toCommunity, toArtist, toNotification, toPost, toComment, toMedia } from ".";
import axios from 'axios';
import { isWeversePasswordAuthorization, createLoginPayload, createRefreshPayload } from "./helpers";
import { ClientNotifications } from "../models";
import EventEmitter from 'events';
/**
 * WeverseEmitter allows the WeverseClient to emit events and provides methods for doing so
 * @class
 */
class WeverseEmitter extends EventEmitter {
    constructor() {
        super();
        /**
...
export class WeverseClient extends WeverseEmitter {
    ...
    constructor(authorization, verbose) {
        ...
        this.notifications = new ClientNotifications();
        this.posts = [];
    ...
    async login(credentials) {
        ...
        try {
            ...
            const response = await axios.post(urls.login, this._loginPayload, { validateStatus });
    ...
    async getCommunityArtists(c, opts) {
        ...
        try {
            ...
            if (await this.handleResponse(response, urls.community(c.id))) {
                ...
                if (data.artists) {
                    const artists = ArtistArray(data.artists).map((a) => toArtist(a, c));
    ...
    async getCommunityPosts(c, pages) {
        ...
        while (count <= pages) {
            try {
                const response = await axios.get(urls.communityPostsPages(wvc.id, from), { headers: this._headers });
                if (await this.handleResponse(response, urls.communityPostsPages(wvc.id, from))) {
                    const data = response.data;
                    if (data.posts) {
                        const newPosts = PostArray(data.posts).map((p) => {
        ...
        while (count < pages) {
            try {
                ...
                if (await this.handleResponse(response, urls.notifications(from))) {
                    const n = NotificationArray(data.notifications).map((n) => {
                        var _a;
                        if (n.communityId === 0)
                            return;
        ...
        try {
            ...
            if (await this.handleResponse(response, urls.postComments(p.id, c.id))) {
                ...
                if (data.artistComments) {
                    const comments = CommentArray(data.artistComments).map((c) => {
    ...
    async checkToken() {
        ...
        try {
            const response = await axios.get(urls.checkToken, { headers: this._headers, validateStatus });
